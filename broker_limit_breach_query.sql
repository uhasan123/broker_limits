...
from
(select * from invoices where created_at<=current_date and created_at>current_date-30 and debtor_id in (select distinct id from cal_debtor_weekly_open_invoice_volume)) a
left join
