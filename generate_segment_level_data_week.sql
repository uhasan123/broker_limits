...
sum(case when (i.approved_date>snapshot_date-7 and i.approved_date<=snapshot_date) and i.approved_date is not null then i.approved_accounts_receivable_amount/100.0 else null end) as invoice_approved_dollars,
count(distinct case when (i.paid_date>snapshot_date-7 and i.paid_date<=snapshot_date) and i.paid_date is not null then i.id else null end) as invoice_paid,
sum(case when (i.paid_date>snapshot_date-7 and i.paid_date<=snapshot_date) and i.paid_date is not null then i.approved_accounts_receivable_amount/100.0 else null end) as invoice_paid_dollars,
avg(case when (i.paid_date>snapshot_date-7 and i.paid_date<=snapshot_date) and i.paid_date is not null then i.dtp else null end) as dtp,
count(distinct case when (i.approved_date>snapshot_date-7 and i.approved_date<=snapshot_date) and i.paid_date is null then i.id else null end) as open_invoices_till_date,
...
count(distinct case when (i.approved_date>snapshot_date-7 and i.approved_date<=snapshot_date) and i.approved_date is not null then i.client_id else null end) as no_of_clients
from mondays m
...
and (i.approved_date >= current_date-455 or i.paid_date >= current_date-455)
and d.status = 'active'
